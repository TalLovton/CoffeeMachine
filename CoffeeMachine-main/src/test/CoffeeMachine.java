...
public class CoffeeMachine {
    ...
    public void menu(){
        ...
        String size = checkTypo(input.next(),"orderSize");
        ...
        for(int i =0; i < orderSize; i++){
            System.out.println("\u2615" + "what coffee would like for order number " + (i+1) + ":?"+"\u2615" + "\n" +
             "Type the exact name:\n" + "Capuccino\n" + "Espresso\n" + "Late\n");
            bevargeType = checkTypo(input.next(),"beverageType");;
            switch(bevargeType) {
                case "Capuccino":
    ...
    private String checkTypo(String next, String text) {
        String res = null;
        switch (text) {
            case "orderSize":
                while (Integer.parseInt(next) < 0) {
                    System.out.println("Your order number is unavailable! Please insert number again: \n");
                    Scanner input = new Scanner(System.in);
                    next = input.next();
                }
                res = next;
                break;
            case "beverageType":
                while(!next.equals("Capuccino") && !next.equals("Late") && !next.equals("Espresso")){
                    System.out.println("Your beverage type is unavailable! Please insert beverage type again: \n");
                    Scanner input = new Scanner(System.in);
                    next = input.next();
                }
                res = next;
                break;
            case "milkType":
                while(!next.equals("Cow") && !next.equals("Soy") && !next.equals("Almond")){
                    System.out.println("Your milk type is unavailable! Please insert milk type again: \n");
                    Scanner input = new Scanner(System.in);
                    next = input.next();
                }
                res = next;
    ...
    public void menuChooseNotEspresso(int index,String bevargeType){
        ...
        milkType  = checkTypo(input.next(),"milkType");;
